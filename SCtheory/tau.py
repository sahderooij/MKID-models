...
def qpstar(kbT, SCsheet, uselowTapprox=False):
    ...
    return SC.t0 * SC.N0 * SC.kbTc**3 / (4 * nqp_ * D_**2) * (1 + SCsheet.tesc / SC.tpb)
...
def scat_kaplan(kbT, SC, uselowTapprox=False):
    ...
    (i.e. phonon absorption) from Kaplan1976 and uses the intral form s.t.
    it holds for all temperatures.
    a^2F(omega)=b omega^2 is assumed."""
    if uselowTapprox:
        return SC.t0 / (
            gamma(7 / 2)
...
def scat_em_kaplan(dE, kbT, SC, uselowTapprox=False):
    """The phonon emission rate of a quasiparticle with energy dE above Delta,
    from eq. (8) of Kaplan1976. If uselowTapprox is True, eq. (17) is used."""
    if uselowTapprox:
        hw = SC.D0 + dE
        return SC.t0 / (
            (SC.D0 / SC.kbTc) ** 3
            * (
                1 / 3 * ((hw / SC.D0) ** 2 - 1) ** (3 / 2)
                + 5 / 2 * ((hw / SC.D0) ** 2 - 1) ** (1 / 2)
                - (
                    SC.D0/ (2 * hw)
                    * (1 + 4 * (hw / SC.D0) ** 2)
                    * np.log(hw / SC.D0 + ((hw / SC.D0) ** 2 - 1) ** (1 / 2))
                )
            )
        )
    else:
        ...
        return (
            ...
            * SC.kbTc**3
            * (1 - SCth.f(hw, kbT))
            / integrate.quad(integrand, 0, dE, args=(hw, D_, kbT))[0]
        )

def scat_abs_kaplan(dE, kbT, SC, uselowTapprox=False):
    """The phonon absorption rate of a quasiparticle with energy dE above Delta,
    from eq. (8) of Kaplan1976."""
    D_ = SCth.D(kbT, SC)
    hw = D_ + dE
    def integrand(E, hw, D_, kbT):
        return (
    ...
    return (
        ...
        * SC.kbTc**3
        * (1 - SCth.f(hw, kbT))
        / integrate.quad(integrand, 0, np.inf, args=(hw, D_, kbT))[0]
    )

def rec_kaplan(dE, kbT, SC, uselowTapprox=False):
    """The recombination rate of a quasiparticle with energy dE above Delta,
    from eq. (8) of Kaplan1976. If uselowTapprox is True, eq. (17) is used."""
    D_ = SCth.D(kbT, SC)
    hw = D_ + dE
    def integrand(E, hw, D_, kbT):
    ...
    return (
        ...
        * SC.kbTc**3
        * (1 - SCth.f(hw, kbT))
        / integrate.quad(integrand, hw + D_, np.inf, args=(hw, D_, kbT))[0]
    )
...
def esc(kbT, tqpstar, SC):
    ...
    D_ = SCth.D(kbT, SC)
    nqp_ = SCth.nqp(kbT, D_, SC)
    return SC.tpb * ((4 * tqpstar * nqp_ * D_**2) / (SC.t0 * SC.N0 * SC.kbTc**3) - 1)
...
def G(x):
    ...
    fldt = theta // (np.pi / 2)
    return (4 / (np.pi - 1)) * (
        np.arctan(np.tan(phi % (np.pi / 2)) / np.tanh(phi))
        + fldp * np.pi / 2
        - 1
        / np.pi
        * (np.arctan(np.tan(theta % (np.pi / 2)) / np.tanh(theta) + fldt * np.pi / 2))
    )
...
def scat_eph2(kbT, SCsheet):
    ...
    return hbar() / (
        sc.D0
        / Z(sc) ** 2
        * (1 + 6 / np.pi * (4 * sc.lbd_eph_clean - 3 / 2) * rhohat)
        * rhohat ** (1 / 2)
        * 9 * np.pi**2 * np.sqrt(6) / 5
        * gamma(7 / 2) * zeta(7 / 2)
        * sc.lbd_eph_clean
        * (sc.cL / sc.cT) ** 4
        * (sc.D0 / sc.kbTD) ** 2
...
def scat_eph_2D(kbT, SCsheet):
    sc = SCsheet.SC
    return _scat_eph_2D(kbT, sc) / (
        db(SCsheet, sc.cL, 2) * sc.vF**2 / sc.cL**2 * gamma(5 / 2) * zeta(5 / 2)
    )
...
def _eph2_3D(kbT, SCsheet):
    ...
    return hbar() / (
        ...
        * (
            rhohat**0.5
            + 6 * (4 * sc.lbd_eph_clean - 3 * mu) / np.pi * rhohat ** (3 / 2)
        )
        * db(SCsheet, sc.cT, 3)
        * 2
        * np.sqrt(6)
...
def recomb_eph2_3D(kbT, SCsheet):
    sc = SCsheet.SC
    return _eph2_3D(kbT, SCsheet) / (np.exp(-sc.D0 / kbT))
...
def _eph2_2D(kbT, SCsheet):
    ...
    Y1 = (
        ...
        * 1e-2
        / SCsheet.d
        / 4108
        * np.log(2 * sc.kF * sc.xi_DL)
        * (
            sc.cL
            * sc.kF
            / (8 * np.pi * sc.kbTD / hbar())
            * sc.lbd_eph_clean
            * (
                1
                - 2
                / np.pi
                * np.arcsin(1 - 0.5 * (sc.kbTD / hbar() / (sc.kF * sc.cL)) ** 2)
            )
            - np.pi**-2
        )
    )
...
def recomb_eph2_2D(kbT, SCsheet):
    sc = SCsheet.SC
    return _eph2_2D(kbT, SCsheet) / (np.exp(-sc.D0 / kbT))
...
def scat_ee_2D_dirty(kbT, SCsheet):
    ...
    kappa = 1/2 * nu * const.e**2 / const.epsilon_0 * 1e12 / const.e 

    # these rates have units of energy
    scatrate42 = (
        kbT / (2 * np.pi**2 * sc.D * nu) * (1 + sc.lbd_eph**2) * np.exp(-sc.D0 / kbT)
    )
    scatrate43 = (
        21
        / (2 * np.pi**6)
    ...
    scatrate45 = (
        (np.pi * sc.D0**2 * kbT) ** (1 / 3)
        / (2 * np.pi**2 * sc.D * nu) ** (3 / 2)
        * np.exp(-2 * sc.D0 / (3 * kbT))
    ) / hbar() ** (3 / 2)

    return hbar() / (scatrate42 + scatrate43 + scatrate45)
...
def recomb_ee_2D_dirty(kbT, SCsheet):
    ...
    return hbar() / (
        kbT
        / (4 * np.pi * sc.D * sc.N0 * 2 * SCsheet.d)
        * (1 + sc.lbd_eph**2)
        * np.exp(-2 * sc.D0 / kbT)
    )
...
def recomb_ee_3D_dirty(kbT, SCsheet):
    ...
    return hbar() / (
        sc.D0**0.5
        * kbT
        / (2 ** (1 / 4) * (2 * np.pi) ** 2 * sc.D ** (3 / 2) * 2 * sc.N0)
        * (1 + sc.lbd_eph**2)
        * np.exp(-2 * sc.D0 / kbT)
        / hbar() ** (3 / 2)
    )
...
def _B(SCsheet):
    sc = SCsheet.SC
    nu = 2 * sc.N0 * SCsheet.d
    kappa = 1/2 * nu * const.e**2 / const.epsilon_0 * 1e12 / const.e # 1/µm (note: Gaussian units in Reizer1998)
    return (
        np.log(4 * sc.EF / ((np.sqrt(8) + np.sqrt(3)) * sc.D0))
        - np.log((2 * sc.kF + kappa) / kappa)
        - 4 * sc.kF / (2 * sc.kF + kappa)
    )

def _A1(SCsheet):
    sc = SCsheet.SC
    return 1 + (1 + 2 / np.pi) * sc.lbd_eph**2

def _A2(SCsheet):
    sc= SCsheet.SC
    return (1 + sc.lbd_eph**2) * (1 + np.sqrt(2) / 4) + np.sqrt(2) * sc.lbd_eph**2
...
def recomb_ee_2D(kbT, SCsheet):
    sc = SCsheet.SC
    return hbar() / (
        _A2(SCsheet) * _B(SCsheet) * kbT * sc.D0 / (2 * sc.EF) * np.exp(-2 * sc.D0 / kbT)
    )

def ee_3D(kbT, SCsheet):
    '''Normal metal electron-electron scattering, for high electron density,
    kappa3 << kF
    Note, the square-root in the definition of kappa_3 is missing in Reizer1998! 
    See Rammer&Smith1986'''
    sc = SCsheet.SC
    kappa3 = np.sqrt(2*sc.N0*const.e**2/ const.epsilon_0 * 1e12 / const.e)
    assert kappa3 < sc.kF/10
    return hbar()/ (
        np.pi**4/64 * kbT**2 / sc.EF * kappa3 / sc.kF
    )

        
def scat_ee_3D(kbT, SCsheet):
    sc = SCsheet.SC
    return ee_3D(kbT, SCsheet) / (
        _A1(SCsheet) * 2 * sc.D0 * np.sqrt(2*np.pi*sc.D0*kbT) / kbT**2 
        * np.exp(-sc.D0/kbT)
    )

def rec_ee_3D(kbT, SCsheet):
    sc = SCsheet.SC
    return ee_3D(kbT, SCsheet) / (
        _A2(SCsheet) * 4 * sc.D0 / kbT
        * np.exp(-2*sc.D0/kbT)
    )   
