        ...
        if any([type(kbT) is float, type(D) is float,
            ...
            for i in range(len(kbT)):
                result[i] = integrate.quad(
                    integrand, D[i], np.inf, args=(kbT[i], D[i], SC))[0]
            return result
...
def calc_Nwsg(kbT,D,e,SC):
    ...
    def integrand(E,kbT,SC):
        return 3*SC.V*E**2/(2*np.pi*(const.hbar/const.e*1e12)**2*(6.3e3)**3*(np.exp(E/kbT)-1))
...
def kbTbeff(tqpstar,SC,plot=False):
    ...
    Nqp_0 = SC.V*SC.t0*SC.N0*SC.kbTc**3/\
        (2*SC.D0**2*tqpstar)*(1+SC.tesc/SC.tpb)/2
...
def nqpfromtau(tau,SC):
    ...
    return SC.t0*SC.N0*SC.kbTc**3/(2*SC.D0**2*2*tau/(1+SC.tesc/SC.tpb))