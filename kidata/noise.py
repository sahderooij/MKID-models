...
def rej_pulses(data,nrsgm=6,nrseg=32,sfreq=50e3,tau=150e-6,plot=False):
    corrdata = subtr_offset(data)
    smdata = smooth(corrdata,tau,sfreq)
        ...
        for j in tnrange(len(Preads),desc='Pread',leave=False):
            ...
            for k in tnrange(len(Temps),desc='Temp',leave=False):
                ...
                spamp,rejectamp = rej_pulses(
                    amp,nrsgm=nrsgm,nrseg=nrseg,tau=tau,sfreq=sfreq)
                spphase,rejectphase = rej_pulses(
                    phase,nrsgm=nrsgm,nrseg=nrseg,tau=tau,sfreq=sfreq)
                #amp:
                ...
                assert all(np.logical_or(lsfRR == lsfPP,np.isnan(lsfRR)))\
                        and all(np.logical_or(lsfPP == lsfPP,np.isnan(lsfPR))), 'different frequency vectors'
