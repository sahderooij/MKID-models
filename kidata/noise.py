...
from kidata import io, calc, filters
            ...
            for k in tnrange(len(Temps), desc="Temp", leave=False):
                ...
                for l, freq in enumerate(freqs):
                    if freq == "med":
                        sfreq = 50e3
                        nperseg = (
                            "seglen"  # optimized to have information in low frequencies
                        )
                    elif freq == "fast":
                        sfreq = 1e6
                    ...
                    allPSDs[l, 0] = np.array(
                        calc_avgPSD(spamp, rejected[l, 0], sfreq=sfreq,
                                    nperseg=nperseg,
                                   dataid=f'{Chipnum}, KID{KIDs[i]}, {Preads[j]} dBm, {Temps[k]} mK, {freq}')
                    ).T
                    # phase:
                    allPSDs[l, 1] = np.array(
                        calc_avgPSD(
                            spphase, rejected[l, 1], sfreq=sfreq,
                            nperseg=nperseg, 
                            dataid=f'{Chipnum}, KID{KIDs[i]}, {Preads[j]} dBm, {Temps[k]} mK, {freq}'
                        )
                    ).T
                ...
                # write to TDparam:
                if all(
                    np.logical_and(PSDs[0][0] == PSDs[1][0], PSDs[1][0] == PSDs[2][0])
                ):
                    for freq in freqs:
...
def calc_avgPSD(
    dataarr, reject, dataarr1=None, reject1=None, sfreq=50e3, nperseg="seglen",
    dataid=None
):
